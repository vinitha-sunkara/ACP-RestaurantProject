...
public class ShoppingCart implements Serializable {
	...
	public double calculatePrice() {
		...
		this.cartItem.forEach(info-> {
			amount[0] = amount[0] + info.getProductInfo().getPrice() * info.getQuantity();
		});
		...
		return Double.valueOf(decimalFormat.format(amount[0]));
	}

	public ItemInfo findItemInCart(String productCode) {

		return this.cartItem.stream().filter(info2 -> info2.getProductInfo().getProductCode().equalsIgnoreCase(productCode))
				.findFirst()
				.map(info2 -> info2).orElse(null);
